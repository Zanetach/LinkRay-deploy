...
- For Mihomo clients, include `meta-rules-dat` `rule-providers` and visible strategy groups when the user wants routing rules. Expose `自动选择`, `故障转移`, `负载均衡`, `节点选择`, `流媒体`, `手动切换`, `全球代理`, `DNS_Proxy`, `Telegram`, `Google`, `YouTube`, `Netflix`, `Spotify`, `HBO`, `Bing`, `OpenAI`, `ClaudeAI`, `Disney`, `GitHub`, `国内媒体`, `本地直连`, and `漏网之鱼`; route each `RULE-SET` to the matching group. Do not add a duplicate English `AUTO` group unless the operator explicitly asks for it. Keep the last rule as `MATCH,漏网之鱼`.
- Put operator/provider/admin/payment dashboards that break through VPS egress behind explicit `DIRECT` rules before `geolocation-!cn`, for example `DOMAIN-SUFFIX,dmit.io,DIRECT`.
...
| Provider, admin, or payment pages work direct but fail after importing the subscription | Add a front-loaded `DIRECT` override such as `DOMAIN-SUFFIX,dmit.io,DIRECT` before `geolocation-!cn` |
